    ...
    C$.inherit('CreasetophModel','').prototype = {
        ...
        navigate: function(url) {
            if(this.ajax_navigation) {
                ...
                this.url = url;
//                $('.main_container_canvas').animate({'opacity':0},null,30000).forward();
                this.ajax_ui(this.base_url + '/' + url + '/ajax/html',{'content':'.main_container_canvas'});
//                $('.main_container_canvas').animate({'opacity':0},null,30000).backward();
            }else {
                this.set_query();
                window.location = this.base_url + '/' + url;
            }
            //TODO: this needs to be pulled out into site specific code
            creasetoph.NavSlide.prototype.set_current(this.query[0]);
        },
